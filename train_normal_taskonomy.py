...
class ConsistentNormal(pl.LightningModule):
    def __init__(self,
        ...
        self.save_hyperparameters(
            ...
            'precision', 'val_check_interval', 'max_epochs', 'load_fragments'
        )
        ...
        self.load_fragments = load_fragments
        self.save_debug_info_on_error = False

        self.setup_datasets()
        self.val_samples = self.select_val_samples_for_datasets2()
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument(
            '--load_fragments', action='store_true', default=False,
            help='Set to load cached fragments.')
        parser.add_argument(
    ...
    def select_val_samples_for_datasets(self):
        ...
        while len(val_imgs['replica']) + len(val_imgs['gso']) + len(val_imgs['taskonomy']) < 85:
            idx = random.randint(0, len(self.valset))
            example = self.valset[idx]
            building = example['positive']['building']
            if building.__contains__('-') and len(val_imgs['gso']) < 45:
                val_imgs['gso'].append(idx)
            elif building.__contains__('_') and not building.__contains__('-') and len(val_imgs['replica']) < 20:
    ...
    def log_ood_example_images(self, data_dir='/datasets/evaluation_ood/real_world/images', num_images=15):
        ...
        for img_idx in range(num_images):
            ...
            with torch.no_grad():
                normal_pred = self.model.forward(rgb.unsqueeze(0))[0]
...
if __name__ == '__main__':
    ...
    if args.experiment_name is None:
        args.experiment_name = 'taskonomy_normal_baseline_v2'
    ...
    if args.restore is None:
        ...
        trainer = Trainer(
            ...
            logger=wandb_logger, checkpoint_callback=checkpoint_callback, accelerator='ddp', distributed_backend='ddp'
        )
