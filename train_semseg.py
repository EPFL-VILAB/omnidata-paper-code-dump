...
class SemanticSegmentation(pl.LightningModule):
    ...
    def shared_step(self, batch, train=True):
        ...
        # Forward pass 
        # labels_preds_taskonomy, labels_preds_replica, labels_preds_hypersim, labels_preds_gso = self(rgb)
        labels_preds = self(rgb)

        losses = {'taskonomy': 0, 'replica': 0, 'hypersim': 0, 'gso': 0}
        counts = {'taskonomy': 0, 'replica': 0, 'hypersim': 0, 'gso': 0}

        total_loss = criterion(labels_preds, labels_gt)

        # for batch_idx in range(len(batch['positive']['rgb'])):
        #     building = batch['positive']['building'][batch_idx]
        #     label_gt = labels_gt[batch_idx].unsqueeze(0)
        #     if building.startswith('ai_'): #hypersim
        #         label_pred = labels_preds[batch_idx].unsqueeze(0)
        #         losses['hypersim'] += criterion(label_pred, label_gt)
        #         counts['hypersim'] += 1
        #     elif building in REPLICA_BUILDINGS: # replica
        #         label_pred = labels_preds[batch_idx].unsqueeze(0)
        #         losses['replica'] += criterion(label_pred, label_gt)
        #         counts['replica'] += 1
        #     elif building_in_gso(building): # gso
        #         label_pred = labels_preds_gso[batch_idx].unsqueeze(0)
        #         losses['gso'] += criterion(label_pred, label_gt)
        #         counts['gso'] += 1
        #     else: #taskonomy
        #         label_pred = labels_preds[batch_idx].unsqueeze(0)
        #         losses['taskonomy'] += criterion(label_pred, label_gt)
        #         counts['taskonomy'] += 1
    ...
    def log_validation_example_images(self, num_images=10):
        ...
        for dataset in self.val_datasets:
            for img_idx in self.val_samples[dataset]:
                ...
                anno_gt = wandb.Image(np.uint8(gt_im), caption=f'GT {img_idx}')
                all_imgs[f'gt-{dataset}'].append(anno_gt)
                anno_pred = wandb.Image(np.uint8(pred_im), caption=f'Pred {img_idx}')
                all_imgs[f'pred-{dataset}'].append(anno_pred)
...
if __name__ == '__main__':
    ...
    if args.restore is None:
        trainer = pl.Trainer.from_argparse_args(args, logger=wandb_logger, \
            checkpoint_callback=checkpoint_callback, gpus=-1, auto_lr_find=False, gradient_clip_val=1, accelerator='ddp')
    else:
        trainer = pl.Trainer(
