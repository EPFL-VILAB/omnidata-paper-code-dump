...
from data.nyu_dataset import NYUDataset, build_mask, mask_val
...
class ConsistentNormal(pl.LightningModule):
    ...
    def setup_datasets(self):
        ...
        self.trainset_nyu =  NYUDataset(root='/datasets/nyu_official', type='train', \
        ...
        print(f'Train set (NYU) contains {len(self.trainset_nyu.imgs)} samples.')
    ...
    def train_dataloader(self):
        return DataLoader(
            self.trainset_nyu, batch_size=self.batch_size, shuffle=True,
    ...
    #     samples_weight = torch.tensor([weight[0]] * taskonomy_count + [weight[1]] * replica_count \
    #          + [weight[2]] * hypersim_count + [weight[3]] * gso_count)
    ...
    def _shared_step(self, batch, train=True):
        ...
        if train:
            rgb = batch[0]
            normal_gt = batch[1]
            mask_valid = normal_gt != mask_val['normal']
            # mask_valid = build_mask(target=normal_gt.cpu(), val=mask_val['normal'])
    ...
    def validation_epoch_end(self, outputs):
        ...
        if self.global_step >= self.log_val_imgs_step + 19999 or self.global_step <= 3000:
    ...
    def log_validation_example_images(self, num_images=20):
        ...
        for img_idx in range(10):
            dataset = 'nyu'
            example = self.trainset_nyu[img_idx]
            num_positive = self.num_positive
            rgb_pos = example[0]
            normal_gt_pos = example[1]

            rgb_pos = rgb_pos.unsqueeze(axis=0).float().to(self.device)
            normal_gt_pos = normal_gt_pos.unsqueeze(axis=0)

            with torch.no_grad():
                normal_preds_pos = self.model.forward(rgb_pos)['normal']
                normal_preds_pos = torch.clamp(normal_preds_pos, -1, 1)
    ...
    def log_ood_example_images(self, data_dir='/datasets/evaluation_ood/real_world/images', num_images=15):
        ...
        for img_idx in range(num_images):
            ...
            with torch.no_grad():
                ...
                normal_pred = torch.clamp(normal_pred, -1, 1)
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.Adam(self.parameters(), lr=1e-4, weight_decay=1e-4, amsgrad=True)
...
if __name__ == '__main__':
    ...
    if args.restore is None:
        trainer = Trainer.from_argparse_args(args, logger=wandb_logger, \
            checkpoint_callback=checkpoint_callback, gpus=[0], auto_lr_find=False, \
                accelerator='ddp', replace_sampler_ddp=False, gradient_clip_val=10)
    else:
        trainer = Trainer(
