...
class ConsistentNormal(pl.LightningModule):
    def __init__(self,
        ...
        self.model = MultiTaskModel(tasks=['principal_curvature'], n_channels=3,\
    ...
    def setup_datasets(self):
        ...
        tasks = ['rgb', 'principal_curvature', 'mask_valid']        
        ...
        self.val_datasets = ['taskonomy', 'replica']
    ...
    def val_dataloader(self):
        ...
        return [taskonomy_dl, replica_dl]

    def forward(self, x):
        return self.model(x)['principal_curvature']

    def training_step(self, batch, batch_idx):
        res = self.shared_step(batch, train=True)
        # Logging
        self.log('train_curv_loss', res['curv_loss'], prog_bar=True, logger=True, sync_dist=self.gpus>1)
        return {'loss': res['curv_loss']}

    def validation_step_combined(self, batch, batch_idx):
        res = self.shared_step(batch, train=False)
        # Logging
        self.log('val_curv_loss', res['loss'], prog_bar=True, logger=True, sync_dist=self.gpus>1)
    ...
    def _shared_step(self, batch, train=True):
        ...
        curv_gt = batch['positive']['principal_curvature']
    ...
    def validation_epoch_end(self, outputs):
        counts = {'taskonomy':0, 'replica':0, 'all':0}
        losses = {}
    ...
    def log_validation_example_images(self, num_images=20):
        ...
        for dataset in self.val_datasets:
            for img_idx in self.val_samples[dataset]:
                ...
                curv_gt_pos = example['positive']['principal_curvature']

                mask_valid = self.make_valid_mask(example['positive']['mask_valid']).squeeze(axis=0).repeat_interleave(3,0)

                curv_gt_pos[~mask_valid] = 0

                rgb_pos = rgb_pos.unsqueeze(axis=0)
                curv_gt_pos = curv_gt_pos.unsqueeze(axis=0)
                ...
                with torch.no_grad():
                    curv_preds_pos = self.model.forward(rgb_pos)['principal_curvature']
                    curv_preds_pos = torch.clamp(curv_preds_pos, -1, 1)
                ...
                for pos_idx in range(num_positive):
                    ...
                    curv_gt = curv_gt_pos[pos_idx].permute(1, 2, 0).detach().cpu().numpy()
                    curv_gt = wandb.Image(curv_gt, caption=f'GT-Curv I{img_idx}')
                    all_imgs[f'gt-curv-{dataset}'].append(curv_gt)

                    curv_pred = curv_preds_pos[pos_idx].permute(1, 2, 0).detach().cpu().numpy()
                    curv_pred = wandb.Image(curv_pred, caption=f'Pred-Curv I{img_idx}')
                    all_imgs[f'pred-curv-{dataset}'].append(curv_pred)
    ...
    def log_ood_example_images(self, data_dir='/datasets/evaluation_ood/real_world/images', num_images=15):
        ...
        for img_idx in range(num_images):
            ...
            with torch.no_grad():
                curv_pred = self.model.forward(rgb.unsqueeze(0))['principal_curvature'][0]
                curv_pred = torch.clamp(curv_pred, -1, 1)

            rgb = rgb.permute(1, 2, 0).detach().cpu().numpy()
            rgb = wandb.Image(rgb, caption=f'RGB OOD {img_idx}')
            all_imgs['rgb_ood'].append(rgb)

            curv_pred = wandb.Image(curv_pred, caption=f'Pred-Curv OOD {img_idx}')
            all_imgs['pred-curv-ood'].append(curv_pred)

        self.logger.experiment.log(all_imgs, step=self.global_step)

    def configure_optimizers(self):
        optimizer = torch.optim.Adam(self.parameters(), lr=3e-5, weight_decay=2e-6, amsgrad=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--save-dir', type=str, default='experiments/curv',
    ...
    if args.experiment_name is None:
        args.experiment_name = 'taskonomy_curv_baseline'
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        verbose=True, monitor='val_curv_loss', mode='min', period=1, save_last=True, save_top_k=3
    )
